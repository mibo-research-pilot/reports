...
If the reporter is unavailable, the tables are still rendered and the narrative sections
carry a neutral "not generated this session" note (never a TODO marker), so the step
degrades gracefully while the committed record stays free of placeholders.
...
# Placeholder used when a narrative section could not be generated. Deliberately not the
# word "TODO": the weekly record is a finished artifact and must never ship TODO markers.
FALLBACK_TEXT = "_Not generated this session._"
...
def call_reporter(reporter: dict, key: str, facts: dict, laws_text: str, data: dict) -> dict:
    prompt = REPORTER_PROMPT.format(
        day=facts.get("day", "?"),
        date=facts.get("date", "?"),
        historical_fact=facts.get("immutable_historical_fact", ""),
        coding_guardrail=facts.get("prospective_coding_guardrail", ""),
...
def as_md(value, default: str = FALLBACK_TEXT) -> str:
...
def build_report(date_s: str, day: int, analysis: dict, meta: dict, queries: list[dict],
    ...
    L.append("")
    L.append(as_md(narrative.get("executive_summary")) if narrative else FALLBACK_TEXT)
    L.append("")
    L.append("---")
    L.append("")
    L.append("## Law Status")
    L.append("")
    L.append(as_md(narrative.get("law_status")) if narrative else FALLBACK_TEXT)
    L.append("")
    L.append("---")
    L.append("")
    ...
    L.append("")
    L.append(as_md(narrative.get("methodological_notes")) if narrative else FALLBACK_TEXT)
    L.append("")
    L.append("---")
    L.append("")
    L.append(f"## Day {day} One-Sentence Conclusion")
    L.append("")
    L.append(as_md(narrative.get("conclusion")) if narrative else FALLBACK_TEXT)
    L.append("")
    return "\n".join(L)
...
def main() -> int:
    ...
    # Narrative: use the reporter model when its key is available; retry once on failure so
    # a transient error does not leave the record without its analysis sections.
    ...
    if reporter and key:
        facts = {
            ...
            "immutable_historical_fact": config.get("immutable_historical_fact"),
            "prospective_coding_guardrail": config.get("prospective_coding_guardrail"),
        }
        ...
        model_note = (f"Automatically generated by `render_observation.py` ({stamp}); entity tables "
                      f"from the coded data. Narrative generation was unavailable this run.")
        for attempt in range(1, 3):
            try:
                ...
                print(f"reporter narrative drafted by {reporter['model']} (attempt {attempt}).")
                break
            except Exception as e:  # noqa: BLE001 - retry, then fall back to tables-only
                print(f"warning: reporter attempt {attempt} failed ({e}).", file=sys.stderr)
                narrative = {}
    else:
        model_note = (f"Automatically generated by `render_observation.py` ({stamp}); entity tables "
                      f"from the coded data. Reporter key absent this run.")
        print("reporter key absent; rendering tables without narrative.")
    ...
    try:
        ...
        print(f"warning: report assembly failed ({e}); rendering tables without narrative.",
              file=sys.stderr)
        report = build_report(date_s, day, analysis, meta, queries, systems, {}, model_note)
    out_path.write_text(report, encoding="utf-8")

    # Keep the dated-folder law artifacts consistent with the report and free of placeholders:
    # write the generated Law Status into law-updates.md and analysis.json (no "TODO" tokens).
    law_text = as_md(narrative.get("law_status")) if narrative else FALLBACK_TEXT
    law_updates = "\n".join([
        f"# MIBO Day {day} — Law Updates", "",
        f"**Observation date**: {date_s}", "",
        "## Law status this session", "", law_text, "",
        "## Candidate propositions", "", "None proposed automatically this session.", "",
    ])
    (data_dir / "law-updates.md").write_text(law_updates, encoding="utf-8")
    try:
        a = json.loads(analysis_path.read_text(encoding="utf-8"))
        a["law_status"] = {"summary": law_text}
        analysis_path.write_text(json.dumps(a, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    except Exception as e:  # noqa: BLE001 - analysis.json law_status is secondary; do not fail the run
        print(f"warning: could not update analysis.json law_status ({e}).", file=sys.stderr)
